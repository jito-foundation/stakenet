...
#[derive(Parser)]
#[command(about = "Closes the directed stake whitelist account")]
pub struct CloseDirectedStakeWhitelist {
    #[command(flatten)]
    pub permissioned_parameters: PermissionedParameters,
}

pub async fn command_close_directed_stake_whitelist(
    args: CloseDirectedStakeWhitelist,
    ...
    let whitelist_account =
        get_directed_stake_whitelist_address(&all_steward_accounts.config_address, &program_id);

    let ix = Instruction {
        program_id,
        accounts: jito_steward::accounts::CloseDirectedStakeWhitelist {
            config: all_steward_accounts.config_address,
            whitelist_account,
            authority: authority.pubkey(),
        }
        .to_account_metas(None),
        data: jito_steward::instruction::CloseDirectedStakeWhitelist {}.data(),
    };
