...
pub fn emit_validator_history_metrics(
    ...
    let epoch_info = &metrics_state.epoch_info;
    let get_vote_accounts = metrics_state.vote_account_map.values().collect::<Vec<_>>();
    let validator_histories = &metrics_state
        .validator_history_map
        .values()
        .collect::<Vec<_>>();
    let cluster_history = &metrics_state.cluster_history;
    ...
    let live_validator_histories_count = metrics_state.get_live_vote_accounts().len();
...
pub fn emit_keeper_stats(metrics_state: &MetricsState) -> Result<(), Box<dyn std::error::Error>> {
    let keeper_balance = metrics_state.keeper_balance;
...
pub fn emit_steward_stats(metrics_state: &MetricsState) -> Result<(), Box<dyn std::error::Error>> {
    //    - Progress
    // - Current State
    // - Num pool validators
    ...
    if metrics_state.all_steward_accounts.is_none() {
        return Ok(());
    }

    let steward_state = &metrics_state
    ...
    let reserve_stake = &metrics_state
        .all_steward_accounts
        .as_ref()
        .unwrap()
        .reserve_stake_account;

    let stake_pool = &metrics_state
        .all_steward_accounts
        .as_ref()
        .unwrap()
        .stake_pool_account;
    ...
    let actual_epoch = metrics_state.epoch_info.epoch;
    ...
    let validator_list_account = &metrics_state
        .all_steward_accounts
        .as_ref()
        .unwrap()
