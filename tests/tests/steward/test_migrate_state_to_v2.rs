...
async fn test_migrate_state_to_v2() {
    ...
    let account_before = fixture.load_and_deserialize::<StewardStateAccount>(&fixture.steward_state).await;
    
    // Verify it has the V1 discriminator
    let account_data_before = fixture.ctx.borrow_mut().get_account(&fixture.steward_state).unwrap().data;
    ...
    let account_after = fixture.load_and_deserialize::<StewardStateAccountV2>(&fixture.steward_state).await;
    
    // Verify it has the V2 discriminator
    let account_data_after = fixture.ctx.borrow_mut().get_account(&fixture.steward_state).unwrap().data;
    let discriminator_after = &account_data_after[0..8];
    assert_eq!(discriminator_after, StewardStateAccountV2::DISCRIMINATOR);
    assert_ne!(discriminator_before, discriminator_after, "Discriminator should have changed");
    
    // Verify data was preserved correctly
    assert_eq!(account_after.state.state_tag, state_tag_before);
    assert_eq!(account_after.state.num_pool_validators, num_validators_before);
    assert_eq!(account_after.state.current_epoch, current_epoch_before);
    assert_eq!(account_after.state.next_cycle_epoch, next_cycle_epoch_before);
    
    // Verify scores were zero-extended from u32 to u64
    for i in 0..10 {
        assert_eq!(account_after.state.scores[i], account_before.state.scores[i] as u64);
        assert_eq!(account_after.state.raw_scores[i], account_before.state.yield_scores[i] as u64);
    }
}
